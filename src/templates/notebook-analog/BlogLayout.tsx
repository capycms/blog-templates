...
export default function BlogLayout({ children, branding }: { children: React.ReactNode; branding?: Branding }) {
  const name = branding?.blogName ?? "Notebook Analog";
  return (
    <div className="bg-yellow-50 bg-[linear-gradient(rgba(0,0,0,0.04)_1px,transparent_1px)] [background-size:100%_28px] text-yellow-900 min-h-screen">
      <header className="border-b border-yellow-300 border-dashed bg-yellow-50/80 backdrop-blur">
        <div className="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="font-serif text-xl font-bold text-yellow-700" style={branding?.accentColor ? { color: branding.accentColor } : undefined}>
