...
export default function BlogLayout({ children, branding }: { children: React.ReactNode; branding?: Branding }) {
  const name = branding?.blogName ?? "GitHub Wiki";
  return (
    <div className="bg-white text-gray-800 min-h-screen">
      <header className="border-b border-gray-300 bg-gray-50/80 backdrop-blur">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="font-mono text-xl font-bold text-blue-600" style={branding?.accentColor ? { color: branding.accentColor } : undefined}>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 flex gap-8">
          ...
          <aside className="w-64 shrink-0 hidden lg:block">
            <div className="sticky top-8 bg-gray-50 border-gray-300 border rounded-lg p-4">
              <h3 className="font-mono text-xl font-bold mb-2">About</h3>
