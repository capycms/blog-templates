...
export default function BlogLayout({ children, branding }: { children: React.ReactNode; branding?: Branding }) {
  const name = branding?.blogName ?? "Thought Leader";
  return (
    <div className="bg-white text-gray-800 min-h-screen">
      <header className="border-b border-gray-200 bg-gray-50/80 backdrop-blur">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="font-serif text-xl font-bold text-indigo-500" style={branding?.accentColor ? { color: branding.accentColor } : undefined}>
        ...
        <div className="flex-1 min-w-0">{children}</div>
          <aside className="w-64 shrink-0 hidden lg:block">
            <div className="sticky top-8 bg-gray-50 border-gray-200 border rounded-lg p-4">
              ...
              <p className="text-sm opacity-70">Thought Leader template</p>
            </div>
          </aside>
      </main>
