...
export default function BlogLayout({ children, branding }: { children: React.ReactNode; branding?: Branding }) {
  const name = branding?.blogName ?? "Asymmetric Bold";
  return (
    <div className="bg-yellow-50 text-gray-900 min-h-screen">
      <header className="border-b border-red-300 bg-white/80 backdrop-blur">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="font-serif text-xl font-bold text-red-500" style={branding?.accentColor ? { color: branding.accentColor } : undefined}>
      ...
      <footer className="border-t border-red-300 mt-12">
