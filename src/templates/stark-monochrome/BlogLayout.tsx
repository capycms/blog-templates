...
export default function BlogLayout({ children, branding }: { children: React.ReactNode; branding?: Branding }) {
  const name = branding?.blogName ?? "Stark Monochrome";
  return (
    <div className="bg-white text-black min-h-screen">
      <header className="border-b border-black bg-white/80 backdrop-blur">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="font-sans text-xl font-bold text-black" style={branding?.accentColor ? { color: branding.accentColor } : undefined}>
